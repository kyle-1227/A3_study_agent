...
def test_real_postgres_lock_serializes_two_independent_instances():
    conninfo = os.getenv("A3_TEST_POSTGRES_URI")
    if not conninfo:
        pytest.skip("A3_TEST_POSTGRES_URI is not configured")

    async def run_lock_scenario() -> None:
    ...
    with asyncio.Runner(loop_factory=postgres_compatible_event_loop) as runner:
        runner.run(run_lock_scenario())
