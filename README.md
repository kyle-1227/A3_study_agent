...
| 真实 canary | 已提供 active PGR 网页 canary 操作规程；本次文档提交未执行真实浏览器 canary，不声明通过 |
...
当前记录的完整后端结果是 `2871 passed / 7 skipped`。Semgrep 与 Gitleaks 当前未安装、未运行，不能写成通过；本次文档提交没有执行或验收真实网页 canary，也不能由单元测试结果替代。
