...
| 质量门 | 后端 `2880 passed / 7 skipped`；前端 36 个文件、`187 passed`，typecheck、lint、production build 通过；Import Linter `3/3` |
...
完整部署、PostgreSQL restart/replay、六场景 Playwright canary 操作规程与恢复边界见 [生产部署运行手册](docs/runbooks/production_deployment.md)。操作规程本身不代表真实 canary 已通过。
...
当前完整后端结果是 `2880 passed / 7 skipped`；前端 36 个测试文件共 `187 passed`，typecheck、lint 和显式 `NEXT_PUBLIC_API_URL=http://localhost:8000` 的 production build 通过；Import Linter `3/3`。Semgrep 与 Gitleaks 当前未安装、未运行，不能写成通过。两轮最终 code-practice 真实浏览器 canary 已通过，但它们不能替代尚未完成的六场景 canary 和人工内容验收。
