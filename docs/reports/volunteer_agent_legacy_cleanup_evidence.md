...
| Context Apply fallback policy 字段 | `fallback_on_error`、`fallback_if_empty_after_drop`、`fallback_to_rule_based` 固定 false，但 schema/config/trace/tests 仍存在 | 保留预算裁剪、明确 dropped reason 与 importance observe-only；证明无运行路径读取这些开关后删除字段、adapter、trace 与专属测试 |
...
- This is a replacement milestone, not the assessment deletion gate. The
  ...
  `assessment_final` stream terminal, PostgreSQL endpoint recovery, and global
  Resource Final V3 switch are still required before deleting the old
  assessment surfaces or the active graph.
