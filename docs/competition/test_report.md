...
发布证据按层记录：`main` 已发布到 `b8f9504`；SSE `eed2139`、Evidence `4a91f68` 与 RAG `f53a710` 是待集成候选。治理和最终 Docker 重建完成前不声明最终 integration SHA，也不把以下工程门禁外推为六场景或人工教育效果通过。

| 项目 | 可声明状态 | 说明 |
| --- | --- | --- |
| 完整后端 pytest | `2880 passed / 7 skipped` | 该次后端基线的 runtime、受控 recovery、Docker 与 hermetic rollout 合同均已纳入；不代表候选 integration 已发布 |
