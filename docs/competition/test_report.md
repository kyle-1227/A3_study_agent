...
发布证据按层记录：比赛演示 runtime source / integration 为 `ca3960a`，已由 `main` 包含并发布；SSE `eed2139`、Evidence `4a91f68` 与 RAG `f53a710` 已分别以 `d7f5802`、`cde3e59`、`fa0f2dc` 集成，治理以 `9cb929c` 集成。最终 Docker 基础复验已完成，但不把以下工程门禁外推为六场景或人工教育效果通过。

| 项目 | 可声明状态 | 说明 |
| --- | --- | --- |
| 完整后端 pytest | `2880 passed / 7 skipped` | 该次后端基线的 runtime、受控 recovery、Docker 与 hermetic rollout 合同均已纳入；不代表六场景或人工教育效果通过 |
| SSE `eed2139` 完整前端门禁 | 36 个文件、`208 passed` | ESLint、typecheck、显式本地 API URL 的 production build 同时通过 |
| Evidence `4a91f68` | `64 passed` | 同 Provider/模型、resource+subject partition bounded reask；reask 不判断 blocked |
| RAG `f53a710` 总控 | `48 passed / 1 skipped` | 同 rerank endpoint、complete-score batch split；禁止 RRF-only/partial scores |
| RAG `f53a710` 车道 | `50 passed / 1 skipped` | 独立车道结果，不与总控数字合并 |
...
| PostgreSQL 重连连接池聚焦测试 | 连接池相关 `60 passed`；checkpointer + security `43 passed` | 严格配置、健康检查、生命周期与 fail-closed 合同通过 |
| 最终 revision 镜像 | runtime source `ca3960a`；backend `sha256:6f7108ce1af9d5124c1e39a1c241d50eea7b55cb591ef784bc965bfe97247d48`；frontend `sha256:a650fd112b6469236def418b4ea136d702b46dbd572a3b389e829b4bf547de5e` | 三容器 healthy；`/`、`/onboarding`、`/health/ready` 均为 HTTP 200 |
| PostgreSQL-only restart | 最终 revision 下仍待单独复验 | 不由容器健康或历史 reconnect 单元测试替代 |
...
| 前端 test/typecheck/lint/build | SSE `eed2139`：36 files、`208 passed`；其余三项通过 | 通过 |
...
| PostgreSQL reconnecting pool 聚焦回归 | 连接池相关 `60 passed`；`tests/test_checkpointer.py` + security 共 `43 passed` | 通过；真实数据库单独重启仍单列复验 |
...
以下 `result.json` 路径是历史本机证据，当前 tracked checkout 和 production
checkout 均未包含这些文件；不得把路径表述为提交包内可下载、可独立复核的产物。
...
- checkpointer 使用严格配置的健康检查重连连接池，初始化失败不降级到 `MemorySaver`；
...
- 允许的生产级受控恢复只有同 Provider/模型的 resource+subject partition bounded reask、同 rerank endpoint 的 complete-score batch split，以及同请求权威 status recovery；每条都有严格触发条件、预算、trace/status 与负例，且不得降低结果质量。
- 请求时仍不得切换 Flat RAG、其他 Provider、其他模型或 generation，也不得把 partial evidence、pending status、未审阅 draft 或空对象伪装为成功。
- 受控 recovery 合同测试必须与正常路径一起通过；未完成真实故障注入时不能把单元测试写成部署验收。
...
在至少两个真实线程完成后记录终态 payload hash、Context injection、artifact URL 和 SSE event identity，再只重启 PostgreSQL。重启前后的 backend/frontend 容器 ID 必须相同；随后确认 readiness 恢复、历史 thread status 未漂移、SSE journal 可在保留窗口内回放、刷新恢复下载卡，并重新下载全部 artifact，要求 HTTP 200、attachment 且非空。两轮已记录 code-practice 线程应合计复验 6 个文件并保持各自 `injection_count=15`。跨用户/跨线程访问仍须拒绝。

仅看到 `/health/ready` 恢复不能判定通过，因为它只证明连接池可借出健康连接。若需要手动重启 backend 才恢复、应用容器 ID 改变、历史状态或 URL 漂移、任何下载失败，则 PostgreSQL-only restart 验收失败。恢复过程不得自动切换 generation、Provider、模型或 Flat RAG。
...
只有低级到高级证据都满足本次发布范围，才能声明比赛演示验收通过。当前记录只支持“各候选工程门禁分别通过、历史 Docker/Provider/code-practice 单场景连续两轮通过”；它不支持“最终 integration 已发布”“完整六场景已通过”“人工学术质量已验收”或“学习效果已经被正式证明”的表述。
